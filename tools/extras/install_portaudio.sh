...
if [ ! -e pa_stable_$VERSION.tgz ]; then
	...
	# https://github.com/PortAudio/portaudio/archive/refs/tags/pa_stable_v19_20111121_r1788.tar.gz
        $WGET -T 10 -t 3 https://github.com/PortAudio/portaudio/archive/refs/tags/pa_stable_$VERSION.tar.gz
    fi

    if [ ! -e pa_stable_$VERSION.tar.gz ]; then
        echo "Download of pa_stable_$VERSION.tar.gz - failed!"
        echo "Aborting script. Please download and install port audio manually!"
        exit 1;
    fi
fi

tar -xovzf pa_stable_$VERSION.tar.gz || exit 1
...
cd portaudio-pa_stable_v19_20111121_r1788
